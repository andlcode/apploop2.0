...
export const LoopManagerProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const updateBeat = () => {
      if (currentAudio || isScheduled) {
        const now = Date.now();
        const syncTime = audioManager.masterSyncTime || now;
        const elapsed = now - syncTime;
        
        if (elapsed < 0) {
          setCurrentBeat(1); // Wait for scheduled start
        } else {
          const durMs = barDuration * 1000;
          if (durMs > 0) {
            const posicaoNoCompasso = elapsed % durMs;
            const batidaAtual = Math.floor((posicaoNoCompasso / durMs) * 4) + 1;
            setCurrentBeat(batidaAtual);
          }
        }
  ...
  }, [currentAudio, isScheduled, barDuration]);
